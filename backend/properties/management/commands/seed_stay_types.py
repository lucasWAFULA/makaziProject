"""
Management command: seed_stay_types

Idempotently seeds (or re-seeds) all PropertyCategory, PropertyType,
and PropertyFeature rows from the production spec.

Usage:
    python manage.py seed_stay_types
    python manage.py seed_stay_types --clear     # wipe and re-seed
"""
...
CATEGORIES = [
    ...
    },
    {
        ...
        "types": [
            ...
            "Family Villa", "Executive Villa", "Garden Villa", "Oceanfront Villa", "Safari Villa",
        ],
    },
    {
    ...
    },
    {
        ...
        "types": [
            ...
            "3 Bedroom Penthouse", "Luxury Penthouse", "Sky Penthouse", "Duplex Penthouse",
        ],
    },
    {
...
class Command(BaseCommand):
    help = "Seed all PropertyCategory, PropertyType, and PropertyFeature rows (idempotent)"

    def add_arguments(self, parser):
        parser.add_argument(
            "--clear",
            action="store_true",
            help="Delete all existing categories, types, and features before re-seeding",
        )

    def handle(self, *args, **options):
        from properties.models import PropertyCategory, PropertyType, PropertyFeature

        if options["clear"]:
            PropertyCategory.objects.all().delete()
            PropertyFeature.objects.all().delete()
            self.stdout.write(self.style.WARNING("Cleared all categories, types, and features."))
        ...
        for cat_data in CATEGORIES:
            ...
            cat, created = PropertyCategory.objects.update_or_create(
                slug=cat_data["slug"],
                defaults=cat_data,
            )
            if created:
            ...
            for i, type_name in enumerate(types):
                ...
                _, t_created = PropertyType.objects.update_or_create(
                    category=cat,
                    slug=type_slug,
                    defaults={"name": type_name, "order": i + 1, "is_active": True},
                )
                if t_created:
                    total_types += 1
        ...
        for feat_data in FEATURES:
            _, created = PropertyFeature.objects.update_or_create(
                slug=feat_data["slug"],
                defaults={
                    "name": feat_data["name"],
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nDone! "
                f"{total_cats} new categories · "
                f"{total_types} new types · "
                f"{total_feats} new features seeded.\n"
                f"Total in DB: "
                f"{PropertyCategory.objects.count()} categories · "
                f"{PropertyType.objects.count()} types · "
                f"{PropertyFeature.objects.count()} features"
            )
        )
